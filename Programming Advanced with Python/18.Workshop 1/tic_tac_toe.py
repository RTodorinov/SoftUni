...
def is_valid_sign(player_one_sign_):
    if player_one_sign_ in ['X', '0']:  # може да се напише на един ред return player_one_sign_ in ['X', '0']
        return True
    return False
...
plyer_one = input("Player one name: ").strip()
plyer_two = input("Player two name: ").strip()

while True:
    player_one_sign = input(f"{plyer_one}, would you like to play with 'X' or '0'?").upper()
    if is_valid_sign(player_one_sign):
        break
    print("Please, enter a one of 'X' of '0'!")
...
print(f"{plyer_one} starts first!")

turn = 1

while True:
    current_player = plyer_one if turn % 2 != 0 else plyer_two
