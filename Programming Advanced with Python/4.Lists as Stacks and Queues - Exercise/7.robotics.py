...
while products:
    ...
    for robot in robots:
        if robot['data'][1] <= start_time_seconds:
            ...
            h = start_time_seconds // 3600  # връщаме времето от секунди в часове
            m = (start_time_seconds % 3600) // 60  # връщаме времето от секунди в минути
            s = (start_time_seconds % 3600) % 60  # връщаме времето от секунди в секунди
            h %= 24  # h % 24
