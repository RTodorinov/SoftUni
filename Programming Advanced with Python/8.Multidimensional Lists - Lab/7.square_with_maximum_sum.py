...
def check_valid_index(row, col_):
    if 0 <= row + 2 <= rows and 0 <= col_ + 2 <= colons:
        return True


def sum_square(row, col_):
    if check_valid_index(row, col_):
        sum_total = 0
        for row_check in range(row, row + 2):
            sum_total += sum(matrix_list[row_check][col_: col_ + 2])
