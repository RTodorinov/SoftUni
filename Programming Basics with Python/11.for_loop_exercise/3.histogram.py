...
# друг процент p2 са от 200 до 399, друг процент p3 са от 400 до 599, друг процент p4 са от 600 до 799 и
...
n = int(input())  # въвеждаме броя литерации
p1, p2, p3, p4, p5 = 0, 0, 0, 0, 0  # добавяме променливи в които ще добавяме

for _ in range(n):
    number = int(input())  # добавяме на всяко завъртане число от конзолата

    if number < 200:   # добавяме числaта според условията в p1,p2,p3,p4,p5
        p1 += 1
    elif number < 400:
        p2 += 1
    elif number < 600:
        ...
        p5 += 1
print(f"{(p1 / n) * 100:.2f}%")  # изчисляваме броя числа разделен на броя литеразии умножено по 100 за процентите.
print(f"{(p2 / n) * 100:.2f}%")
print(f"{(p3 / n) * 100:.2f}%")
print(f"{(p4 / n) * 100:.2f}%")
print(f"{(p5 / n) * 100:.2f}%")
